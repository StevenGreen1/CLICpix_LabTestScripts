...
resultsString = 'Results for SET' + setNumber + '\n'
resultsString += 'SET\tPixel\tFrameNumber\tToT\tToTPlusx\tToTPlusxPlusy\tToTPlusy\tToTMinusxPlusy\tToTMinusx\tPulseHeight\tPulseHeightFromFit\tTroughStart\tTroughTime\t10%TimeStart\t10%TimeEnd\t90%TimeStart\t90%TimeEnd\tFrameCount\n'
...
for dataFileName in sorted(search):
    ...
    regex = re.compile("SET(.*?)/Pixel(.*?)/(.*?)/frame(.*?).dat")
    q = regex.search(dataFileName)

    if q is not None:
        pixelNumber = q.group(2)
        eventNumber = int(q.group(4))
...
#                plt.savefig('SET' + setNumber + '_Frame' + str(eventNumber) + '.png')
                ...
                resultsString += setNumber + '\t' + str(pixelNumber) + '\t' + str(eventNumber) + '\t' + str(totDict[(int(pixelNumber),int(eventNumber),'target')]) + '\t' + str(totDict[(int(pixelNumber),int(eventNumber),'targetPlusx')]) + '\t' + str(totDict[(int(pixelNumber),int(eventNumber),'targetPlusxPlusy')]) + '\t' + str(totDict[(int(pixelNumber),int(eventNumber),'targetPlusy')]) + '\t' + str(totDict[(int(pixelNumber),int(eventNumber),'targetMinusxPlusy')]) + '\t' + str(totDict[(int(pixelNumber),int(eventNumber),'targetMinusx')]) + '\t' + str(troughVoltageChange) + '\t' + str(troughVoltageChangeFromFit) + '\t' + str(startTroughTime) + '\t' + str(troughTime) + '\t' + str(lowPlateauStartTime) + '\t' + str(lowPlateauEndTime) + '\t' + str(plateauStartTime) + '\t' + str(plateauEndTime) + '\t' + str(frameCountDict[(int(pixelNumber),int(eventNumber))]) + '\n'
...
#                print count
                ...
                if (count % 1000 == 0):
                    print 'For SET ' + setNumber + ' (Current Pixel ' + str(pixelNumber) + ') processed ' + str(count) + ' .dat files.'

                if (count % 5000 == 0):
                    resultsFileName = 'PixelCalibration_SET' + setNumber + '_Detailed_v2.dat'
